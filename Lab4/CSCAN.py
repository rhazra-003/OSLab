# Python3 program to demonstrate C-SCAN Disk Scheduling algorithm
...
def CSCAN(arr, head):
	...
	# Tracks on the left of the head will be serviced when once the head comes back to the beggining (left end)
	for i in range(size):
		if (arr[i] < head):
			left.append(arr[i])
		if (arr[i] > head):
